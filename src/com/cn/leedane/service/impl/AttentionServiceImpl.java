 ...
 * Version 1.0
 */
public class AttentionServiceImpl extends BaseServiceImpl<AttentionBean> implements AttentionService<AttentionBean>{
	Logger logger = Logger.getLogger(getClass());
	
	private AttentionDao<AttentionBean> attentionDao;
	
	public void setAttentionDao(AttentionDao<AttentionBean> attentionDao) {
	...
	public Map<String, Object> deleteAttention(JSONObject jo, UserBean user,
		...
		AttentionBean attentionBean = attentionDao.findById(aid);
		if(attentionBean != null && attentionBean.getCreateUser().getId() == createUserId){
			result = attentionDao.delete(attentionBean);
		}else{
			message.put("message", EnumUtil.getResponseValue(EnumUtil.ResponseCode.操作对象不存在.value));
			message.put("responseCode", EnumUtil.ResponseCode.操作对象不存在.value);
		}
