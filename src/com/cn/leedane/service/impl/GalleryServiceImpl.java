 ...
 * 图库service实现类
 * @author LeeDane
 * 2016年1月15日 下午3:16:44
 * Version 1.0
 */
public class GalleryServiceImpl extends BaseServiceImpl<GalleryBean> implements GalleryService<GalleryBean> {
	Logger logger = Logger.getLogger(getClass());
	@Resource
	private GalleryDao<GalleryBean> galleryDao;
	
	@Override
	public boolean isExist(UserBean user, String path) {
		return galleryDao.isExist(user, path);
	}
	...
	public Map<String, Object> delete(JSONObject jo, UserBean user,
		...
		GalleryBean galleryBean = galleryDao.findById(galleryId);
		if(galleryBean == null){
		...
		boolean result = galleryDao.delete(galleryBean);
		if(result){
			message.put("isSuccess", true);
		}else{
	...
	public void setGalleryDao(GalleryDao<GalleryBean> galleryDao) {
