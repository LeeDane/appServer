 ...
 * 赞service的实现类
 * @author LeeDane
 * 2016年1月13日 下午2:31:48
 * Version 1.0
 */
public class ZanServiceImpl extends BaseServiceImpl<ZanBean> implements ZanService<ZanBean>{
	Logger logger = Logger.getLogger(getClass());
	
	private ZanDao<ZanBean> zanDao;

	public void setZanDao(ZanDao<ZanBean> zanDao) {
		this.zanDao = zanDao;
	}
	...
	public Map<String, Object> addZan(JSONObject jo, UserBean user,
		...
		bean.setFroms(froms);
		boolean result = zanDao.save(bean);
		if(result){
			int createUserId = 0;
			String str = "{from_user_remark}点赞，源自："+content;
			createUserId = zanDao.getObjectCreateUserId(tableName, tableId);
	...
	public Map<String, Object> deleteZan(JSONObject jo, UserBean user,
		...
		ZanBean zanBean = zanDao.findById(zid);
		if(zanBean != null && zanBean.getCreateUser().getId() == createUserId){
			result = zanDao.delete(zanBean);
		}else{
			message.put("message", EnumUtil.getResponseValue(EnumUtil.ResponseCode.操作对象不存在.value));
			message.put("responseCode", EnumUtil.ResponseCode.操作对象不存在.value);
		}
