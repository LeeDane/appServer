...
public class BlogTest extends BaseTest {
	...
	public void addDigest(){
		...
		List<Map<String, Object>> idList = blogService.executeSQL("select id from "+DataTableType.博客.value+" where id > ?", 2);
	...
	public void deleteOne(int index){
		List<Map<String, Object>> list = blogService.executeSQL("select id, title from "+DataTableType.博客.value+" where id > ? order by id asc limit 1", index);
		if(list.size() > 0){
			int preId = StringUtil.changeObjectToInt(list.get(0).get("id"));
			List<Map<String, Object>> list1 = blogService.executeSQL("select id from "+DataTableType.博客.value+" where title=? and id > ?", list.get(0).get("title"), preId);
			if(list1.size()> 0){
